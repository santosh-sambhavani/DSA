 ...
 */
function solveV1(board: string[][]): void {
    const stack: number[][] = [];
    const queue: number[][] = [];
...
/** Approach
  * Note all Os on edge & change it to visited = 1 & all the Os which can connect with it.
  * Than simply change remaining Os connected to X in to X which are not visited
 */
function solveV2(board: string[][]): void {
    const stack: number[][] = [];
    const queue: number[][] = [];
    const rows = board.length;
    const cols = board[0].length;
